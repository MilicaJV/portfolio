...
function createSkillsFromJSON1() {
    const container = document.querySelector("#skills .container");

    // Création du carrousel Bootstrap
    const carousel = document.createElement("div");
    carousel.id = "skillsCarousel";
    carousel.classList.add("carousel", "slide");
    carousel.setAttribute("data-bs-ride", "carousel");
    carousel.setAttribute("data-bs-interval", "3000"); // auto-défilement toutes les 3 sec
    ...
    fetch("data/skills.json")
        ...
        .then((data) => {
            let slide;
            data.forEach((item, index) => {
                // Si c'est le début d'une nouvelle série de 3
                if (index % 3 === 0) {
                    slide = document.createElement("div");
                    slide.classList.add("carousel-item");
                    if (index === 0) slide.classList.add("active"); // première slide active
                ...
                col.classList.add("col-md-4"); // 3 colonnes
                col.innerHTML = `
                    <div class="card skillsText mb-4">
                        <div class="card-body text-center">
                            <img src="./images/${item.image}" class="d-block w-100" alt="${item.title}">
                            <h3 class="card-title mt-3">${item.title}</h3>
                            <p class="card-text mt-3">${item.text}</p>
                        </div>
                    </div>
                ...
                // Ajout de la carte dans la slide en cours
                slide.querySelector(".row").appendChild(col);
            });

            // Assemblage final du carrousel
